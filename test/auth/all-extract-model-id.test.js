...
describe('Extract modelId', () => {
  ...
  routes.forEach(({ route, methods }) => {
    methods.forEach(({ method, tests }) => {
      if (!tests.extractModelId) return
      it(`${method.toUpperCase()} ${route(routeDesc)}`, done => {
        Promise.all([
          request[method](route({ ...routeParams, projectId: project.project.id, modelId: unvalidId }))
            .set('AccessToken', auth.accessToken.token)
            .expect(404, { message: 'ModelId is not valid' }),
