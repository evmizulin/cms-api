...
describe('Extract userId', () => {
  ...
  routes.forEach(({ route, methods }) => {
    methods.forEach(({ method, tests }) => {
      if (!tests.extractUserId) return
      it(`${method.toUpperCase()} ${route(routeDesc)}`, done => {
        Promise.all([
          request[method](route({ ...routeParams, projectId: project.project.id, userId: unvalidId }))
            .set('AccessToken', auth.accessToken.token)
            .expect(404, { message: 'UserId is not valid' }),
